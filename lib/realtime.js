...
export const friendsTopic = (userId) => `friends:${userId}`;    // friend list + requests (useFriends)
export const supportUserTopic = (userId) => `support_user:${userId}`; // a user's ticket list + unread
export const supportTicketTopic = (ticketId) => `support_ticket:${ticketId}`; // open conversation (both sides)
export const supportAdminTopic = () => `support_admin`;          // any new ticket/message (admin console)

// Voice-call signaling topics. UNLIKE the topics above, these carry CLIENT-TO-CLIENT
// broadcast (the browser .send()s directly on the channel — see hooks/useCall.js),
// NOT server-originated pings. No table RLS is involved (WebRTC SDP/ICE is exchanged
// peer-to-peer, never stored), so anon clients sending broadcast on a public channel
// is fine. `call:<userId>` is the user's always-on "doorbell" for incoming invites;
// `rtc:<callId>` is the per-session channel both peers join for the SDP/ICE handshake.
export const callTopic = (userId) => `call:${userId}`;   // per-user incoming-call doorbell
export const rtcTopic = (callId) => `rtc:${callId}`;     // per-call SDP/ICE exchange
