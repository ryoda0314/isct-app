...
// or legacy machine-derived key for backward compat with existing credentials
export const LOCAL_PASSPHRASE = (() => {
  ...
  // Existing credentials present → use legacy derivation for backward compat
  try {
    fs.accessSync(CRED_FILE);
    console.warn(
      '[SECURITY] CRED_SECRET not set. Using legacy key derivation. ' +
      'Set CRED_SECRET env var for better security.'
    );
    return crypto
      .createHash('sha256')
