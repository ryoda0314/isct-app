...
// H10: Validate regex patterns are compilable and reject on load
...
async function getWords() {
  ...
  // Filter out invalid regex patterns to avoid runtime errors
  _cache = (data || []).filter(w => w.match_type !== 'regex' || isValidRegex(w.word));
  _cacheTime = now;
  return _cache;
}
...
export async function checkNgWords(text, context) {
  ...
  // H10: Truncate text to prevent ReDoS on very long inputs
  ...
  for (const w of words) {
    ...
    if (w.match_type === 'exact') hit = lower === w.word.toLowerCase();
      ...
      try {
        // H10: Execute regex with a length-limited input to mitigate ReDoS
        const re = new RegExp(w.word, 'i');
        hit = re.test(truncated);
      } catch {}
    } else {
