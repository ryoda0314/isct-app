...
export async function normalizeTextbooks({ year, dept, faculty } = {}) {
  ...
  // 1. Load raw rows matching filter — paginate to bypass Supabase 1000-row default
  const PAGE = 1000;
  const rawRows = [];
  for (let from = 0; ; from += PAGE) {
    let q = sb.from('course_textbooks_raw').select('*').eq('syllabus_year', year)
    ...
    if (error) {
      ...
      throw new Error(`load raw: ${error.message}`);
    }
