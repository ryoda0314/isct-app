...
export async function enrichTextbooks({ year, dept, faculty, useGoogleBooks } = {}) {
  ...
  // Load orphan course_books for this scope
  ...
  const { data: orphans, error: orphErr } = await q;
  if (orphErr) {
    enrichProgress.delete(progressKey);
    throw new Error(`load orphans: ${orphErr.message}`);
  }

  enrichProgress.set(progressKey, { phase: 'searching', total: orphans.length, done: 0 });
  ...
  for (const row of orphans) {
    ...
    if (done % 5 === 0) {
      enrichProgress.set(progressKey, { phase: 'searching', total: orphans.length, done });
    }
  }
