...
function inferSingleCategory(norm) {
  ...
  if (cats.length !== 1) {
    return { breakdown: null, total_percent: null, has_breakdown: false, is_pass_fail: false };
  }
  const cat = cats[0];
...
function cleanLabel(s) {
  if (!s) return '';
  let r = s;
  // 文頭の箇条書き番号 ("1)", "(1)", "1.", "①") を最初に削除して
  // 後段の「対応無し `)` 打ち切り」が箇条書きの `)` を誤検出するのを防ぐ
  r = r.replace(/^\s*[(（]?\s*[\d０-９①-⑨ⅠⅡⅢⅣⅤⅥⅦⅧⅨⅩ]+\s*[)）.．、:：]\s*/, '');
  // ラベル末尾の (およそ / (約 / 程度 を除去
  ...
  // 前後の不要文字: 記号と空白のみを文字クラスで削除する
  // (※「および」「または」のような接続詞を文字クラスに入れると、ラベル冒頭の
  //  「お」「ま」など個別の仮名を1文字単独で削ってしまうバグになるため、
  //  接続詞はキーワード扱いで下段で処理する)
  r = r.replace(/^[\s,、。\.・\/／+＋\-—–:：;；()（）&]+/, '');
  r = r.replace(/[\s,、。\.・\/／+＋\-—–:：;；(（&]+$/, '');
  // 文頭の接続詞/助詞 (キーワードとして1回だけ)
  r = r.replace(/^(?:および|及び|または|又は|あるいは|なお|また|さらに|加えて|並びに|による|などの|等の|から算出|や|と|成績は|評価は|主に|総合的に|基本的に|本講義の|本講座の|の)/, '').trim();
  // 文末の助詞/動詞 (キーワードとして1回だけ)
  r = r.replace(/(?:による|により|を含む|に基づき|から算出|に応じて|の合算|の合計|の総合|を総合|で評価|で計算)$/, '').trim();
  // 末尾の単独助詞「を/から/が/は/に/の」 — 誤削除を避けるためラベル長 5+ で1文字だけ
  if (r.length >= 5) {
    r = r.replace(/(?:を|から|が|は|に|の)$/, '').trim();
  }
  ...
  // ラベル中央に対応する閉じ括弧が無い開き括弧があれば、そこで打ち切る
  // "その成果物(グループ毎" → "その成果物"  (注釈括弧が % で途切れた残り)
  {
    const openIdx = r.search(/[(（]/);
    if (openIdx > 0) {
      const after = r.slice(openIdx);
      if (!/[)）]/.test(after)) {
        r = r.slice(0, openIdx).trim();
      }
    }
  }
...
export function parseGradingBreakdown(text) {
  ...
  // 合否科目 (Pass/Fail) 検出: 「合格・不合格で評価」「合否で判定」等
  const isPassFail = /(?:合格\s*[・／\/、,と又は]+\s*不合格|合\s*否(?:のみ|で|を)?|合格\s*又は\s*不合格|pass\s*[\/／or]\s*(?:no\s*pass|fail))/i.test(norm);
  ...
  // % が一つも無ければ「N点」表記を試す (合計 50-200 点なら % にスケール)
  if (!/\d\s*%/.test(norm)) {
    const pointRe = /(\d{1,3})\s*点/g;
    const points = [];
    let pm;
    while ((pm = pointRe.exec(norm)) !== null) {
      points.push({ idx: pm.index, end: pm.index + pm[0].length, value: parseFloat(pm[1]) });
    }
    if (points.length >= 2) {
      const total = points.reduce((s, p) => s + p.value, 0);
      if (total >= 50 && total <= 200) {
        // 各「N点」を等価の「M%」に書き換えて以降の処理に乗せる
        let rebuilt = '';
        let cur = 0;
        for (const p of points) {
          rebuilt += norm.slice(cur, p.idx);
          const pct = Math.round(p.value / total * 1000) / 10;
          rebuilt += `${pct}%`;
          cur = p.end;
        }
        rebuilt += norm.slice(cur);
        norm = rebuilt;
      }
    }
  }
  ...
  for (const p of positions) {
    ...
    // 文中導入句を後ろ側から削る
    label = label.replace(/^(?:理解度を|成績は|評価は|評価方法は|評価基準は|主に|総合的に|前半は|後半は|前期は|後期は)/, '').trim();
    // 「Xを〜する」述語パターン: 「期末試験を重視する」(80%) → "期末試験"
    // 動詞: 評価/採点/判定/重視/考慮/加味/算定/算出/対象とする/材料とする 等
    {
      const verb = label.match(/^(.+?)を(?:評価(?:の対象とする)?|採点|判定|重視|考慮|加味|算定|算出|対象と(?:し|する)|材料と(?:し|する)|根拠と(?:し|する)|総合(?:的に)?(?:評価|判定))(?:する|して|します|る)?$/);
      if (verb && verb[1].trim().length >= 2) {
        label = verb[1].trim();
      }
    }

    // ラベルが長すぎる場合、助詞境界で末尾セグメントだけ残す
    // ("〜の理解度を期末試験" → "期末試験"、"各授業におけるレポート" → "レポート")
    // - 時間・場所節 (時に/中に/最後に等) は境界としない
    // - 括弧内の助詞 ("予習シートを含む" の「を」) も境界としない
    //   → 括弧の中身をプレースホルダ文字に置換した文字列で tail マッチを試す
    if (label.length > 8) {
      const masked = label.replace(/[(（][^()（）]*[)）]/g, m => '_'.repeat(m.length));
      // 「における」「において」のような連結助詞を優先
      const longJoint = masked.match(/(?:における|において|に関する|に対する|に基づく|に応じた)([^をでにより]{2,30})$/);
      if (longJoint && longJoint[1].trim().length >= 2) {
        const start = masked.length - longJoint[1].length;
        label = label.slice(start).trim();
      } else {
        const tail = masked.match(/(?:を|で|により|は|について|(?<!時|中|最後|最初|当初|同時|常)に)([^をでにより]{2,30})$/);
        if (tail && tail[1].trim().length >= 2) {
          const start = masked.length - tail[1].length;
          label = label.slice(start).trim();
        }
      }
    }
  ...
  if (items.length === 0) {
    return { breakdown: null, total_percent: null, has_breakdown: false, is_pass_fail: isPassFail };
  }
