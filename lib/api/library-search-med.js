...
const ENRICH = 20;     // 所蔵を詳細から取得する件数（全件）
const POOL = 6;        // 詳細取得の並列数
...
function evalHoldings() {
  ...
  // 所蔵館（複数館に所蔵があれば両方）
  const campuses = [...new Set((seg.match(/御茶ノ水|国府台|湯島/g) || []))];
  ...
  // 貸出状況は詳細ページの定型文を誤検出しやすく信頼できないため出さない（館・配架・請求記号のみ）
  return { campus, campuses, location, electronic, call };
}
...
export async function searchMedCatalog(input = {}) {
  ...
  try {
    ...
    if (list.length) {
      ...
      // 残り全件（最大 ENRICH）の所蔵を並列プールで取得
      if (tmpl) {
        const queue = list.slice(0, ENRICH).filter((b) => b.bib && !(b.bib in holdings));
        let idx = 0;
        const worker = async () => {
          while (idx < queue.length) {
            ...
            try {
              ...
              await p.goto(tmpl(b.bib), { waitUntil: 'domcontentloaded' });
              await p.waitForFunction(() => /所蔵一覧/.test(document.body.innerText), { timeout: 9000 }).catch(() => {});
    ...
    const records = list.map((b) => {
      ...
      const campusList = h ? [...new Set((h.campuses || [h.campus]).map(normCampus).filter(Boolean))] : [];
      ...
      return {
        ...
        year,
        detailUrl,
        holdings: h && campusList.length
          ? { campuses: campusList, location: h.location || null, callNumber: h.call || null, electronic: !!h.electronic }
          : (h === null ? null : undefined),
      };
    });
