...
export async function fetchGrades({ userId, password, matrix }) {
  ...
  try {
    ...
    const linkInfo = await page.evaluate(() => {
      ...
      const all = links.map(a => ({ text: (a.textContent || '').trim().substring(0, 80), href: a.href || '' }));
      // Search with broad keywords
      const keywords = ['教務', 'kyomu', 'gakumu', '学務', '成績'];
      let found = null;
    ...
    console.log(`[Grades] Portal links (${linkInfo.all.length}):`, JSON.stringify(linkInfo.all.slice(0, 20)));

    if (linkInfo.found) {
      console.log(`[Grades] Found kyomu link on portal: ${linkInfo.found}`);
      await page.goto(linkInfo.found, { waitUntil: 'networkidle2' });
      await new Promise(r => setTimeout(r, 2000));
      console.log(`[Grades] After kyomu SSO: ${page.url()}`);
    } else {
      // Fallback: try portal SSO redirect pattern
      console.log(`[Grades] No kyomu link found, trying portal SSO redirect...`);
      const ssoUrl = `https://portal.nap.gsic.titech.ac.jp/GetAccess/Login?Template=idg_key&AUTHMETHOD=IdgKey&GAREASONCODE=13&GARESOURCEID=resourcelistID2&GAURI=https://portal.nap.gsic.titech.ac.jp/GetAccess/ResourceList&Reason=13&APPID=kyomu0_gakumu_titech_ac_jp&URI=${encodeURIComponent('https://kyomu0.gakumu.titech.ac.jp/')}`;
      await page.goto(ssoUrl, { waitUntil: 'networkidle2' });
      await new Promise(r => setTimeout(r, 2000));
      console.log(`[Grades] After SSO redirect: ${page.url()}`);
    }
