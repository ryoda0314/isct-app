  ...
  for (const table of tables) {
    ...
    for (const row of rows) {
      ...
      if (texts.length === 2 && /^\d+$/.test(texts[1]) && !['小計', '合計'].includes(texts[0]) && texts[0] !== '科目区分') {
        result.categories.push({ name: texts[0].replace(/\s*※$/, ''), credits: parseFloat(texts[1]) });
        continue;
      }
...
export async function fetchGrades({ userId, password, matrix }) {
  ...
  try {
    ...
    // Override window.open so portal SSO stays in same tab (portal JS uses window.open, not target="_blank")
    await page.evaluate(() => {
    ...
    if (kyomuLinkIndex >= 0) {
      console.log(`[Grades] Clicking kyomu link (index ${kyomuLinkIndex}) with window.open override...`);
      await Promise.all([
      ...
      console.log(`[Grades] After SSO click: ${currentUrl}`);

      // If portal JS still opened a new tab despite override, find it
      if (!currentUrl.includes('kyomu') && !currentUrl.includes('gakumu')) {
        const allPages = await browser.pages();
        if (allPages.length > 1) {
          console.log(`[Grades] ${allPages.length} tabs open, checking for kyomu...`);
          for (const p of allPages) {
            const u = p.url();
            if ((u.includes('kyomu') || u.includes('gakumu')) && u !== 'about:blank') {
              console.log(`[Grades] Using kyomu tab: ${u}`);
              page = p;
              currentUrl = u;
              break;
            }
      ...
      // Detect maintenance page — extract schedule and bail out early
      if (currentUrl.includes('Maintenance')) {
        const maint = await page.evaluate(() => {
          const body = document.body?.innerText || '';
          // Extract schedule lines like "2026/3/17(火) 4:00 ～ 8:50"
          const schedules = body.match(/\d{4}\/\d{1,2}\/\d{1,2}\([^\)]+\)\s*\d{1,2}:\d{2}\s*[～~〜-]\s*\d{1,2}:\d{2}/g) || [];
          // Extract recurring info like "毎週火曜日・金曜日の4:00-8:50"
          const recurring = body.match(/毎週.+/)?.[0] || '';
          return { schedules, recurring, text: body.substring(0, 500) };
        });
        console.log(`[Grades] Maintenance detected:`, JSON.stringify(maint));
        const err = new Error('MAINTENANCE');
        err.maintenance = {
          schedules: maint.schedules,
          recurring: maint.recurring,
        };
        throw err;
      }

      // Navigate to grades within the established kyomu session
      if (currentUrl.includes('kyomu') || currentUrl.includes('gakumu')) {
        console.log(`[Grades] On kyomu, navigating to grades...`);
    ...
    // If still on portal after all attempts, SSO failed
    if (finalUrl.includes('portal.nap.gsic.titech.ac.jp')) {
      throw new Error('教務Webシステムに接続できませんでした。');
    }
