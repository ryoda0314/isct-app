...
/**
 * Extract the submission settings the app needs to build an in-app submit UI:
 * which plugins are enabled (file/text), file constraints, and the flags that
 * decide whether in-app submit is supported vs. must fall back to the LMS.
 *
 * The raw `configs` array holds one entry per plugin setting, e.g.
 *   { plugin:'file', subtype:'assignsubmission', name:'filetypeslist', value:'.pdf' }
 */
function extractSubmissionConfig(asgn) {
  const configs = asgn.configs || [];
  const get = (plugin, name) =>
    configs.find(c => c.subtype === 'assignsubmission' && c.plugin === plugin && c.name === name)?.value;
  return {
    file: get('file', 'enabled') === '1',
    text: get('onlinetext', 'enabled') === '1',
    fileTypes: get('file', 'filetypeslist') || '',            // "" = any type
    maxFiles: Number(get('file', 'maxfilesubmissions')) || 0,  // 0 = unknown
    maxBytes: Number(get('file', 'maxsubmissionsizebytes')) || 0,
    drafts: !!asgn.submissiondrafts,          // needs an explicit submit_for_grading step
    team: !!asgn.teamsubmission,              // group submission — not supported in-app yet
    statement: !!asgn.requiresubmissionstatement,
    cutoff: asgn.cutoffdate || 0,             // hard cutoff (0 = none); after it, no submission
  };
}
...
export function transformAssignments(moodleResponse, courseIdMap) {
  ...
  for (const course of (moodleResponse.courses || [])) {
    ...
    for (const asgn of (course.assignments || [])) {
      // Assignments with no deadline (duedate 0/absent) are kept with due:null
      // instead of being dropped, so they still surface in the list ("期限なし").
      const hasDue = !!asgn.duedate && asgn.duedate !== 0;
      ...
      assignments.push({
        ...
        due: hasDue ? new Date(asgn.duedate * 1000) : null,
        pri: hasDue ? determinePriority(asgn.duedate) : 'low',
        st: 'not_started',
        type: determineType(asgn.name, asgn.intro),
        pts: asgn.grade || 0,
        attachments: extractAttachments(asgn),
        subCfg: extractSubmissionConfig(asgn),
        subs: [],
        cmt: []
      });
    }
  ...
  // Sort by due date ascending; deadline-less assignments sort to the end.
