...
/**
 * Parse all quarters a course spans from a syllabus quarter string.
 * Handles "1Q" → [1], "1-2Q"/"3～4Q" (range) → [1,2]/[3,4],
 * "1・3Q" (non-contiguous) → [1,3]. Returns [] when nothing parses.
 * Courses held across multiple quarters (e.g. B1 物理学演習 = 1-2Q) must
 * appear in every quarter, not just the first.
 */
function parseQuarters(qStr) {
  if (!qStr) return [];
  const range = String(qStr).match(/(\d)\s*[-–～〜~]\s*(\d)/);
  if (range) {
    const a = parseInt(range[1]), b = parseInt(range[2]);
    if (a <= b) { const out = []; for (let q = a; q <= b; q++) out.push(q); return out; }
    return [a, b];
  }
  const digits = String(qStr).match(/\d/g);
  return digits ? [...new Set(digits.map(Number))] : [];
}
...
export function transformCourses(moodleCourses, scheduleMap = {}, userDept = null) {
  ...
  const allCourses = moodleCourses
    ...
    .map((mc) => {
      ...
      // `quarters` lists every quarter the course spans (multi-quarter courses
      // like 1-2Q must show in both); `quarter` is the primary (first) one,
      // kept for callers that expect a single value.
      let quarters = syllabus.quarter ? parseQuarters(syllabus.quarter) : [];
      if (!quarters.length) quarters = [detectQuarter(mc.shortname, mc.fullname)];
...
export function groupByQuarter(courses) {
  ...
  for (const c of courses) {
    // Multi-quarter courses (e.g. 1-2Q) are filed under every quarter they span.
    const qs = (c.quarters && c.quarters.length) ? c.quarters : [c.quarter || 2];
    for (const q of qs) {
