  ...
  const allCourses = moodleCourses
    ...
    .map((mc) => {
      ...
      return {
        ...
        _lastaccess: mc.lastaccess || 0,
      };
    });

  // Deduplicate: when multiple sections share the same course code
  // (e.g. LAL.S204 with S11,S12,S13,...), keep only the one the user
  // most recently accessed — that is most likely their actual section.
  const byCode = new Map();
  for (const c of allCourses) {
    const existing = byCode.get(c.code);
    if (!existing || c._lastaccess > existing._lastaccess) {
      byCode.set(c.code, c);
    }
  }

  return [...byCode.values()].map((c, i) => {
    const { _lastaccess, ...rest } = c;
