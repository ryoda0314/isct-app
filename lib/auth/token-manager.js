...
export async function getToken(loginId) {
  ...
  try {
    ...
    if (dbToken?.wstoken) {
      console.log(`[TokenManager] Found cached token in DB for ${loginId}, validating...`);
      const siteInfo = await validateToken(dbToken.wstoken);
      if (siteInfo) {
        ...
        console.log(`[TokenManager] DB token valid for ${loginId} (user ${entry.userid})`);
        return { wstoken: entry.wstoken, userid: entry.userid, fullname: entry.fullname };
      }
      console.log(`[TokenManager] DB token invalid for ${loginId}, falling back to SSO`);
    }
    ...
    const siteInfo = await validateToken(entry.wstoken);
    if (!siteInfo) {
