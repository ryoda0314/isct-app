...
export async function performSSOLogin({ userId, password, totpSecret, target }) {
  ...
  try {
    ...
    // Debug: dump page state to diagnose selector failures
    const landedHtml = await page.content();
    console.log('[SSO] Page title:', await page.title());
    console.log('[SSO] Has input#identifier:', landedHtml.includes('id="identifier"') || landedHtml.includes("id='identifier'"));
    console.log('[SSO] HTML snippet (first 1000):', landedHtml.substring(0, 1000).replace(/\s+/g, ' '));
    ...
    // Step 2: Enter username and submit to proceed to password step
    await page.waitForSelector('input#identifier', { visible: true });
    await page.type('input#identifier', userId, { delay: 30 });

    // Check if password field exists on the same page
    const hasPasswordOnSamePage = await page.evaluate(() => !!document.querySelector('input#password'));

    if (hasPasswordOnSamePage) {
      // Single-page login: make password field visible and fill it
      await page.evaluate(() => {
    ...
    // Click the submit button (not form.submit()) to trigger JS event handlers
    console.log('[SSO] Submitting login form via button click...');
    await Promise.all([
      ...
      page.evaluate(() => {
        ...
        if (!form) return;
        // Try submit button first, then fall back to form.submit()
    ...
    // If password was on a separate step, handle it now
    if (!hasPasswordOnSamePage) {
      console.log('[SSO] Multi-step login: waiting for password field...');
      await page.waitForSelector('input#password', { visible: true, timeout: SEL_TIMEOUT });
      await page.type('input#password', password, { delay: 30 });
      ...
      await new Promise(r => setTimeout(r, 2000));
      console.log(`[SSO] After password submit, URL: ${page.url()}`);
    }

    // Dump page state before waiting for TOTP to aid debugging
    const postLoginHtml = await page.content();
    console.log('[SSO] Post-login page title:', await page.title());
    console.log('[SSO] Has input#totp:', postLoginHtml.includes('id="totp"') || postLoginHtml.includes("id='totp'"));
    console.log('[SSO] Post-login HTML snippet (first 1000):', postLoginHtml.substring(0, 1000).replace(/\s+/g, ' '));

    // Step 4: TOTP - wait for the input to appear after login page transition
    const totpCode = generateTOTP(totpSecret);
    console.log('[SSO] Waiting for TOTP input...');
    ...
    console.log('[SSO] TOTP entered, submitting...');

    // Click the submit button for TOTP form
    await Promise.all([
      page.waitForNavigation({ waitUntil: 'domcontentloaded', timeout: NAV_TIMEOUT }).catch(() => {}),
      page.evaluate(() => {
