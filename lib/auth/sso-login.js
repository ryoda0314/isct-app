...
async function getBrowser(retries = 3) {
  for (let attempt = 1; attempt <= retries; attempt++) {
    try {
      ...
      // SSO_HEADFUL=1 → open a visible Chrome window (+slowMo) to watch the login flow.
      const headful = process.env.SSO_HEADFUL === '1';
      return await puppeteer.launch({
        headless: headful ? false : 'new',
        slowMo: headful ? (parseInt(process.env.SSO_SLOWMO, 10) || 50) : 0,
