...
export async function performPortalLogin({ userId, password, matrix }) {
  ...
  try {
    ...
    try {
      ...
      // Check the current URL to distinguish password error vs network/timeout
      const url = page.url();
    ...
    if (!finalUrl.includes('ResourceList')) {
      const err = new Error(`Portal login may have failed. Final URL: ${finalUrl}`);
      err.failedStep = 'matrix';
      throw err;
    }
    ...
    const wrapped = new Error(`Portal login failed: ${error.message}`);
    wrapped.failedStep = error.failedStep || 'unknown';
    throw wrapped;
  } finally {
