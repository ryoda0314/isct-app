...
const uDue=d=>{const df=d-NOW,dy=Math.floor(df/864e5),hr=Math.floor((df%864e5)/36e5);if(df<0)return{t:"期限切れ",c:T.red,u:true};if(dy===0){const mn=Math.floor((df%36e5)/6e4);return{t:hr>0?`${hr}h${mn}m`:`${mn}m`,c:T.red,u:true};}if(dy<=2)return{t:`${dy}d${hr}h`,c:T.orange,u:true};if(dy<=7)return{t:`${dy}日`,c:T.yellow};return{t:`${dy}日`,c:T.green};};
