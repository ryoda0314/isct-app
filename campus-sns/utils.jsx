...
const fDS=d=>d?`${d.getMonth()+1}/${d.getDate()}`:"";
const fDF=d=>d?`${d.getMonth()+1}/${d.getDate()} ${d.getHours()}:${String(d.getMinutes()).padStart(2,"0")}`:"";
const uDue=d=>{if(!d)return{t:t("utils.noDeadline"),c:T.txD};const df=d-NOW,dy=Math.floor(df/864e5),hr=Math.floor((df%864e5)/36e5);if(df<0)return{t:t("utils.overdue"),c:T.red,u:true};if(dy===0){const mn=Math.floor((df%36e5)/6e4);return{t:hr>0?`${hr}h${mn}m`:`${mn}m`,c:T.red,u:true};}if(dy<=2)return{t:`${dy}d${hr}h`,c:T.orange,u:true};if(dy<=7)return{t:t("utils.days",{n:dy}),c:T.yellow};return{t:t("utils.days",{n:dy}),c:T.green};};
