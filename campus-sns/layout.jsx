...
const SideItem=({icon,label,on,click,badge,compact})=>(
  <button onClick={click} title={compact?label:undefined} style={{width:"100%",display:"flex",alignItems:"center",justifyContent:compact?"center":"flex-start",gap:compact?0:8,padding:compact?"7px 0":"7px 10px",borderRadius:8,border:"none",cursor:"pointer",background:on?`${T.accent}14`:"transparent",color:on?T.txH:T.tx,fontSize:12.5,fontWeight:on?600:400,textAlign:"left",borderLeft:on?`2px solid ${T.accent}`:"2px solid transparent",position:"relative"}}>
    ...
    {badge===null?<span style={{minWidth:compact?14:16,height:compact?14:16,display:"flex",alignItems:"center",justifyContent:"center",position:compact?"absolute":"static",top:compact?2:undefined,right:compact?2:undefined}}><span style={{width:10,height:10,borderRadius:"50%",border:`1.5px solid ${T.accent}`,borderTopColor:"transparent",animation:"mnSpin .6s linear infinite",display:"block"}}/></span>:badge>0&&<span style={{minWidth:compact?14:16,height:compact?14:16,borderRadius:8,background:T.red,color:"#fff",fontSize:compact?7:9,fontWeight:700,display:"flex",alignItems:"center",justifyContent:"center",padding:"0 3px",position:compact?"absolute":"static",top:compact?2:undefined,right:compact?2:undefined}}>{badge}</span>}
  </button>
);
...
const MNav=({view,setView,ac,unreadN,dmUnread})=>{
  ...
  <nav className="mnav" style={{display:"flex",background:T.bg2,borderTop:`1px solid ${T.bd}`,flexShrink:0}}>
    {[{id:"home",i:I.home,l:"ホーム"},{id:"timetable",i:I.cal,l:"時間割"},{id:"tasks",i:I.tasks,l:"課題",b:ac},{id:"navigation",i:I.map,l:"マップ"},{id:"dm",i:I.mail,l:"DM",b:dmUnread},{id:"moreMenu",i:I.more,l:"その他",b:unreadN}].map(n=>{
      ...
      return <button key={n.id} onClick={()=>setView(n.id)} style={{flex:1,display:"flex",flexDirection:"column",alignItems:"center",justifyContent:"center",gap:2,border:"none",background:"transparent",color:on?T.accent:T.txD,cursor:"pointer",padding:0,position:"relative",height:50}}><div style={{position:"relative"}}>{n.i}{n.b===null?<span style={{position:"absolute",top:-3,right:-6,width:14,height:14,display:"flex",alignItems:"center",justifyContent:"center"}}><span style={{width:8,height:8,borderRadius:"50%",border:`1.5px solid ${T.accent}`,borderTopColor:"transparent",animation:"mnSpin .6s linear infinite",display:"block"}}/></span>:n.b>0&&<span style={{position:"absolute",top:-4,right:-7,minWidth:14,height:14,borderRadius:7,background:T.red,color:"#fff",fontSize:8,fontWeight:700,display:"flex",alignItems:"center",justifyContent:"center",padding:"0 3px"}}>{n.b}</span>}</div><span style={{fontSize:10,fontWeight:on?600:400}}>{n.l}</span></button>;
    })}
  </nav>);
};
