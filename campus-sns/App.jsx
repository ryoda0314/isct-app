...
export default function App(){
  ...
  const mBack=goBack;
  return(
    <div ref={el=>{if(!el)return;const u=()=>{let h=window.innerHeight;if(!isNative()&&!navigator.standalone){const t=document.createElement('div');t.style.height='env(safe-area-inset-bottom,0px)';document.body.appendChild(t);h-=parseFloat(getComputedStyle(t).height)||0;document.body.removeChild(t);}el.style.height=h+"px";};u();window.addEventListener("resize",u);}} style={{display:"flex",flexDirection:"column",width:"100%",overflow:"hidden",background:T.bg,color:T.tx,fontFamily:"'Inter',-apple-system,BlinkMacSystemFont,'Hiragino Sans','Segoe UI',sans-serif",position:"fixed",top:0,left:0,right:0}}>
