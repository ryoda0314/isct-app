...
export default function App(){
  ...
  const mBack=goBack;
  return(
    <div ref={el=>{if(!el)return;el._dbg=el;if(!isNative())return;const u=()=>{el.style.height=window.innerHeight+"px";};u();window.addEventListener("resize",u);}} style={{display:"flex",flexDirection:"column",width:"100%",overflow:"hidden",background:T.bg,color:T.tx,fontFamily:"'Inter',-apple-system,BlinkMacSystemFont,'Hiragino Sans','Segoe UI',sans-serif",position:"fixed",inset:0}}>
      {/* DEBUG: 原因調査用 - あとで削除 */}
      <div ref={el=>{if(!el)return;const d=()=>{const r=el.parentElement.getBoundingClientRect();const t=document.createElement('div');t.style.height='env(safe-area-inset-bottom,0px)';document.body.appendChild(t);const sab=parseFloat(getComputedStyle(t).height)||0;document.body.removeChild(t);el.textContent=`ih:${innerHeight} sh:${screen.height} ch:${r.height.toFixed(0)} cw:${r.width.toFixed(0)} sab:${sab} pwa:${!!navigator.standalone} vvh:${window.visualViewport?.height?.toFixed(0)||'?'}`;};d();addEventListener('resize',d);}} style={{position:"fixed",bottom:0,left:0,right:0,zIndex:99999,background:"rgba(0,0,0,.85)",color:"#0f0",fontSize:11,fontFamily:"monospace",padding:"6px 10px",pointerEvents:"none"}}/>
