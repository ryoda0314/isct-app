...
const TL={pdf:'PDF',slide:'todaymat.type_slide',document:'todaymat.type_document',spreadsheet:'todaymat.type_spreadsheet',image:'todaymat.type_image',video:'todaymat.type_video',audio:'todaymat.type_audio',archive:'todaymat.type_archive',code:'todaymat.type_code',text:'todaymat.type_text',link:'todaymat.type_link',file:'todaymat.type_file'};
const PREVIEWABLE=new Set(['pdf','image','video','audio']);
/* .docx も MatView 側で docx-preview プレビュー可能(canPreview と判定を揃える)。 */
  ...
  return(
    <div style={{borderRadius:8,background:T.bg2,border:`1px solid ${T.bd}`,overflow:"hidden",marginBottom:4}}>
      <div onClick={()=>{if(totalFiles>0)setOpen(o=>!o);}} style={{display:"flex",alignItems:"center",gap:8,padding:"8px 10px",cursor:totalFiles>0?"pointer":"default",borderLeft:`3px solid ${course.col||T.accent}`}}>
        ...
        {loading?<span style={{fontSize:10,color:T.txD}}>{t("common.loading")}</span>
        :error?<span style={{fontSize:10,color:"#ef4444"}}>{t("todaymat.fetch_failed")}</span>
        :<span style={{fontSize:10,color:T.txD,flexShrink:0}}>{t("todaymat.count",{n:totalFiles})}</span>}
      ...
      {open&&allMats.length>0&&<div style={{borderTop:`1px solid ${T.bd}`,padding:"4px 0"}}>
        {allMats.map(mat=>{
          ...
          const tlRaw=TL[mat.fileType]||"todaymat.type_file";
          const tl=tlRaw.startsWith("todaymat.")?t(tlRaw):tlRaw;
  ...
  return(
    <div style={{padding:"4px 16px 8px"}}>
      <div style={{display:"flex",alignItems:"center",gap:6,marginBottom:6}}>
        ...
        <span style={{fontWeight:700,color:T.txH,fontSize:14}}>{t("todaymat.title")}</span>
      </div>
