  ...
  return(
    <div style={{flex:1,overflowY:"auto",padding:mob?12:20}}>
      ...
      <div style={{display:"grid",gridTemplateColumns:"repeat(3,1fr)",gap:8,marginBottom:16}}>
        <div style={{padding:12,borderRadius:10,background:`${T.accent}08`,border:`1px solid ${T.accent}20`,textAlign:"center"}}><div style={{fontSize:24,fontWeight:700,color:T.accent}}>--</div><div style={{fontSize:11,color:T.txD}}>累積GPA</div></div>
        <div style={{padding:12,borderRadius:10,background:`${T.green}08`,border:`1px solid ${T.green}20`,textAlign:"center"}}><div style={{fontSize:24,fontWeight:700,color:T.green}}>{courses.length*2}</div><div style={{fontSize:11,color:T.txD}}>今期単位</div></div>
        <div style={{padding:12,borderRadius:10,background:`${T.orange}08`,border:`1px solid ${T.orange}20`,textAlign:"center"}}><div style={{fontSize:24,fontWeight:700,color:T.orange}}>{courses.length}</div><div style={{fontSize:11,color:T.txD}}>履修科目</div></div>
      </div>
      ...
      {grades.map(g=>{const co=courses.find(c=>c.id===g.cid);const a=att[g.cid]||{};const rate=a.total?Math.round(a.attended/a.total*100):0;return(
        <div key={g.cid} style={{padding:12,borderRadius:10,background:T.bg2,border:`1px solid ${T.bd}`,marginBottom:6,borderLeft:`3px solid ${co?.col||T.txD}`}}>
          <div style={{display:"flex",justifyContent:"space-between",marginBottom:4}}>
            <div><span style={{fontWeight:700,color:co?.col,fontSize:12}}>{co?.code}</span><span style={{fontSize:13,color:T.txH,fontWeight:500,marginLeft:6}}>{co?.name}</span></div>
          </div>
          <div style={{display:"flex",gap:12,fontSize:12,color:T.txD}}>
