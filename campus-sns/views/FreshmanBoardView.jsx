...
// ── Categories ──
const CATEGORIES=[
  {id:"course_reg",labelKey:"freshman.catCourseReg",icon:I.clip,color:"#6366f1",descKey:"freshman.catCourseRegDesc"},
  {id:"circle",labelKey:"freshman.catCircle",icon:I.circle,color:"#f59e0b",descKey:"freshman.catCircleDesc"},
  {id:"campus_life",labelKey:"freshman.catCampusLife",icon:I.home,color:"#10b981",descKey:"freshman.catCampusLifeDesc"},
];
...
const RoleBadge=({yr})=>{
  ...
  return <span style={{fontSize:10,fontWeight:600,padding:"1px 5px",borderRadius:4,
    ...
    whiteSpace:"nowrap"}}>{isSenpai?t("freshman.yearN",{yr}):t("freshman.year1")}</span>;
};
...
const LoginPrompt=({msg,onLogin})=>(
  <div style={{display:"flex",alignItems:"center",gap:8,padding:"10px 14px",borderRadius:10,background:T.bg3,border:`1px solid ${T.bd}`,marginTop:8}}>
    ...
    <span style={{fontSize:12,color:T.txD,flex:1}}>{msg||t("freshman.loginToPostComment")}</span>
    {onLogin&&<span onClick={onLogin} style={{fontSize:12,color:T.accent,fontWeight:600,cursor:"pointer",whiteSpace:"nowrap"}}>{t("freshman.login")}</span>}
  </div>
);
      ...
      {loggedIn?(
        <div style={{display:"flex",gap:6,alignItems:"center",marginTop:4}}>
          <input value={txt} onChange={e=>setTxt(e.target.value)}
            ...
            placeholder={t("freshman.writeReply")}
        ...
        <LoginPrompt msg={t("freshman.loginToComment")} onLogin={onLogin}/>
      )}
    </div>
  );
};
...
// ── Loading spinner ──
const Spinner=()=>(
  <div style={{textAlign:"center",padding:40,color:T.txD,fontSize:13}}>{t("common.loading")}</div>
);
...
export const FreshmanBoardView=({mob,loggedIn,onLogin})=>{
  ...
  const handleToggleLike=useCallback((postId)=>{
    if(!loggedIn){showToast(t("freshman.loginToLike"));return;}
      ...
      <div style={{flex:1,display:"flex",flexDirection:"column",overflow:"hidden"}}>
        ...
        <div style={{flexShrink:0,display:"flex",alignItems:"center",gap:8,padding:"10px 16px",borderBottom:`1px solid ${T.bd}`,background:T.bg2}}>
          ...
          {c&&<div style={{display:"flex",alignItems:"center",gap:4}}>
            ...
            <span style={{fontSize:12,color:c.color,fontWeight:600}}>{t(c.labelKey)}</span>
          </div>}
          <span style={{fontSize:12,color:T.txD}}>{t("freshman.thread")}</span>
        </div>
          ...
          <div style={{padding:mob?16:24}}>
            ...
            <div style={{display:"flex",alignItems:"center",gap:10,marginBottom:14}}>
              ...
              {threadPost.pinned&&<div style={{marginLeft:"auto",display:"flex",alignItems:"center",gap:3,padding:"3px 8px",borderRadius:6,background:`${T.accent}12`,color:T.accent,fontSize:11,fontWeight:600}}>
                <span style={{display:"flex"}}>{I.pin}</span>{t("freshman.pinned")}
              </div>}
            </div>

            {/* Tags */}
            <div style={{display:"flex",gap:6,marginBottom:12,flexWrap:"wrap"}}>
              {c&&<Tag color={c.color}>{t(c.labelKey)}</Tag>}
              {threadPost.type==="question"&&<Tag color="#f59e0b">{t("freshman.question")}</Tag>}
            </div>
            ...
            <div style={{display:"flex",gap:16,alignItems:"center",marginTop:16,paddingTop:12,borderTop:`1px solid ${T.bd}`}}>
              ...
              <div style={{display:"flex",alignItems:"center",gap:4,color:T.txD,fontSize:13,padding:"4px 10px",borderRadius:16,background:T.bg3,border:`1px solid ${T.bd}`}}>
                ...
                <span>{t("freshman.repliesCount",{n:threadPost.commentCount||0})}</span>
              </div>
            </div>

            {/* Comments / replies */}
            <div style={{marginTop:16}}>
              <div style={{fontSize:13,fontWeight:600,color:T.txH,marginBottom:8}}>{t("freshman.replies")}</div>
      ...
      <div style={{flex:1,display:"flex",flexDirection:"column",overflow:"hidden"}}>
        <div style={{flex:1,overflowY:"auto",WebkitOverflowScrolling:"touch",padding:mob?12:20}}>
          ...
          <div style={{display:"flex",alignItems:"center",gap:8,marginBottom:mob?12:16,padding:"0 2px"}}>
            <div style={{fontSize:mob?15:17,fontWeight:700,color:T.txH}}>{t("freshman.boardTitle",{year:2026})}</div>
            <div style={{fontSize:11,color:T.txD}}>{t("freshman.boardSubtitle")}</div>
          </div>
                ...
                <div key={c.id} onClick={()=>setCat(c.id)}
                  ...
                  <div style={{padding:mob?"14px 14px 12px":"18px 18px 14px"}}>
                    <div style={{display:"flex",alignItems:"center",gap:8,marginBottom:8}}>
                      ...
                      <div style={{flex:1}}>
                        <div style={{fontWeight:700,fontSize:14,color:T.txH}}>{t(c.labelKey)}</div>
                        <div style={{fontSize:11,color:T.txD,marginTop:1}}>{t("freshman.threadsCount",{n:count})}</div>
                      </div>
                      <span style={{color:T.txD,display:"flex"}}>{I.arr}</span>
                    </div>
                    <div style={{fontSize:12,color:T.txD,lineHeight:1.5}}>{t(c.descKey)}</div>
                  </div>
                </div>
              );
            })}
          ...
          {loading?<Spinner/>:<>
                ...
                <div key={c.id} style={{marginBottom:mob?16:20}}>
                  <div style={{display:"flex",alignItems:"center",justifyContent:"space-between",marginBottom:8,padding:"0 2px"}}>
                    <div style={{display:"flex",alignItems:"center",gap:6}}>
                      ...
                      <span style={{fontWeight:700,fontSize:14,color:T.txH}}>{t(c.labelKey)}</span>
                      <span style={{fontSize:11,color:T.txD}}>{t("freshman.latestThreads")}</span>
                    </div>
                    <div onClick={()=>setCat(c.id)} style={{fontSize:12,color:T.accent,cursor:"pointer",fontWeight:500,display:"flex",alignItems:"center",gap:2}}>
                      {t("freshman.seeAll")}<span style={{display:"flex"}}>{I.arr}</span>
                    </div>
                  </div>
            ...
            {posts.length===0&&!loading&&(
              <div style={{textAlign:"center",padding:40,color:T.txD,fontSize:13}}>{t("freshman.emptyFirstPost")}</div>
            )}
          </>}
        </div>
      </div>
    ...
    <div style={{flex:1,display:"flex",flexDirection:"column",overflow:"hidden"}}>
      ...
      <div style={{flexShrink:0,borderBottom:`1px solid ${T.bd}`}}>
        ...
        <div style={{padding:mob?"12px 14px":"14px 20px",display:"flex",alignItems:"center",gap:10}}>
          ...
          <div style={{flex:1}}>
            <div style={{fontWeight:700,fontSize:15,color:T.txH}}>{t(activeCat.labelKey)}</div>
            <div style={{fontSize:11,color:T.txD,marginTop:1}}>{t(activeCat.descKey)}</div>
          </div>
        </div>
      ...
      <div style={{flexShrink:0,padding:mob?"8px 12px":"10px 20px",borderBottom:`1px solid ${T.bd}`,display:"flex",gap:8,alignItems:"center",flexWrap:"wrap"}}>
        <div style={{display:"flex",alignItems:"center",gap:4,flex:1,minWidth:120,padding:"5px 10px",borderRadius:8,background:T.bg3,border:`1px solid ${T.bd}`}}>
          ...
          <input value={searchQ} onChange={e=>setSearchQ(e.target.value)} placeholder={t("freshman.searchPlaceholder",{cat:t(activeCat.labelKey)})}
        ...
        <div style={{display:"flex",gap:2,background:T.bg3,borderRadius:8,padding:2,border:`1px solid ${T.bd}`}}>
          {[{id:"new",l:t("freshman.sortNew")},{id:"popular",l:t("freshman.sortPopular")}].map(s=>(
        ...
        {loggedIn?(
          ...
          <div onClick={onLogin} style={{display:"flex",alignItems:"center",gap:4,padding:"5px 12px",borderRadius:8,background:T.bg3,color:T.txD,fontSize:12,fontWeight:500,border:`1px solid ${T.bd}`,cursor:onLogin?"pointer":"default"}}>
            ...
            <span>{t("freshman.loginToPost")}</span>
          </div>
        )}
      </div>
      ...
      {composing&&<>
        ...
        <div style={{position:"fixed",top:"50%",left:"50%",transform:"translate(-50%,-50%)",width:mob?"calc(100% - 32px)":480,maxHeight:"80vh",background:T.bg2,border:`1px solid ${T.bd}`,borderRadius:16,zIndex:999,boxShadow:"0 20px 60px rgba(0,0,0,.4)",display:"flex",flexDirection:"column",overflow:"hidden"}}>
          <div style={{padding:"14px 18px",borderBottom:`1px solid ${T.bd}`,display:"flex",alignItems:"center",justifyContent:"space-between"}}>
            <span style={{fontWeight:700,fontSize:15,color:T.txH}}>{t("freshman.newThread")}</span>
          ...
          <div style={{padding:18,flex:1,overflowY:"auto"}}>
            ...
            {!cat&&<div style={{marginBottom:12}}>
              <div style={{fontSize:12,color:T.txD,marginBottom:6,fontWeight:500}}>{t("freshman.category")}</div>
            ...
            <textarea value={txt} onChange={e=>setTxt(e.target.value)} placeholder={t("freshman.composePlaceholder")} autoFocus
          ...
          <div style={{padding:"12px 18px",borderTop:`1px solid ${T.bd}`,display:"flex",justifyContent:"flex-end",gap:8}}>
            <Btn onClick={()=>{setComposing(false);setTxt("");}}>{t("common.cancel")}</Btn>
            <Btn on onClick={handleSendPost} style={{borderRadius:10,opacity:txt.trim()?1:.4,padding:"6px 20px"}}>{t("freshman.submitPost")}</Btn>
          </div>
        </div>
      </>}
        ...
        {loading?<Spinner/>:<>
          {filtered.length===0&&(
            <div style={{textAlign:"center",padding:40,color:T.txD,fontSize:13}}>
              {searchQ?t("freshman.noMatchingThreads"):t("freshman.noThreadsYet")}
            </div>
          )}
          {filtered.map(p=>{
            const liked=p.likes.includes(userId);
                ...
                <div style={{display:"flex"}}>
                  ...
                  <div style={{flex:1,padding:mob?"12px 12px":"14px 16px"}}>
                    ...
                    <div style={{display:"flex",alignItems:"center",gap:6,marginBottom:6,flexWrap:"wrap"}}>
                      ...
                      {isQ&&<span style={{fontSize:10,fontWeight:700,color:"#f59e0b",background:"#f59e0b18",padding:"1px 6px",borderRadius:4}}>{t("freshman.question")}</span>}
                    </div>
          ...
          {hasMore&&!loadingMore&&(
            <div onClick={loadMore} style={{textAlign:"center",padding:12,color:T.accent,fontSize:13,cursor:"pointer",fontWeight:500}}>{t("freshman.loadMore")}</div>
          )}
          {loadingMore&&<Spinner/>}
        </>}
        <div style={{height:20}}/>
