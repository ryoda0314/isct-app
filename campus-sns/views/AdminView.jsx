...
const PostsTab = () => {
  ...
  const load = useCallback((p, q) => {
    ...
    fetch(`${API}/api/admin?action=posts&page=${p}${qs}`)
      .then(r => r.json())
  ...
  return (
    <div style={{ padding: 16 }}>
      <div style={{ display: "flex", alignItems: "center", gap: 12, marginBottom: 12 }}>
        <div style={{ fontSize: 16, fontWeight: 700, color: T.txH, flex: 1 }}>投稿管理 ({total})</div>
        <div style={{ display: "flex", alignItems: "center", gap: 6, padding: "6px 10px", borderRadius: 8, background: T.bg3, border: `1px solid ${T.bd}`, width: 220 }}>
          <span style={{ color: T.txD, display: "flex" }}>{I.search}</span>
          <input value={search} onChange={e => setSearch(e.target.value)} onKeyDown={e => { if (e.key === "Enter") load(0, search); }} placeholder="検索..." style={{ flex: 1, border: "none", background: "transparent", color: T.txH, fontSize: 13, outline: "none" }} />
        </div>
      </div>
      {loading && <div style={{ color: T.txD, fontSize: 13 }}>読み込み中...</div>}
        ...
        {posts.map(p => (
          <div key={p.id} style={{ padding: 14, borderRadius: 12, background: T.bg3, border: `1px solid ${T.bd}` }}>
            <div style={{ display: "flex", alignItems: "center", gap: 8, marginBottom: 8 }}>
              ...
              <span style={{ fontSize: 11, color: T.txD }}>{p.course_id}</span>
...
const MessagesTab = () => {
  ...
  const load = useCallback((p, q) => {
    setLoading(true);
    const qs = q ? `&search=${encodeURIComponent(q)}` : "";
    fetch(`${API}/api/admin?action=messages&page=${p}${qs}`)
      .then(r => r.json())
  ...
  return (
    <div style={{ padding: 16 }}>
      <div style={{ display: "flex", alignItems: "center", gap: 12, marginBottom: 12 }}>
        <div style={{ fontSize: 16, fontWeight: 700, color: T.txH, flex: 1 }}>メッセージ管理 ({total})</div>
        <div style={{ display: "flex", alignItems: "center", gap: 6, padding: "6px 10px", borderRadius: 8, background: T.bg3, border: `1px solid ${T.bd}`, width: 220 }}>
          <span style={{ color: T.txD, display: "flex" }}>{I.search}</span>
          <input value={search} onChange={e => setSearch(e.target.value)} onKeyDown={e => { if (e.key === "Enter") load(0, search); }} placeholder="検索..." style={{ flex: 1, border: "none", background: "transparent", color: T.txH, fontSize: 13, outline: "none" }} />
        </div>
          ...
          <div key={m.id} style={{ display: "flex", alignItems: "flex-start", gap: 10, padding: 12, borderRadius: 10, background: T.bg3, border: `1px solid ${T.bd}` }}>
            ...
            <div style={{ flex: 1, minWidth: 0 }}>
              <div style={{ display: "flex", alignItems: "center", gap: 6 }}>
                ...
                <span style={{ fontSize: 11, color: T.txD }}>{m.course_id}</span>
