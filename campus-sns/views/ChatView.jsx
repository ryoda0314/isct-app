  ...
  return(
    <div style={{display:"flex",flexDirection:"column",gap:5,marginTop:6,maxWidth:320}}>
      {(multi||anon)&&<div style={{display:"flex",gap:6,flexWrap:"wrap"}}>
        {multi&&<span style={{fontSize:10,color:T.txD,background:T.bg4,padding:"1px 6px",borderRadius:4}}>{t("chat.pollMulti")}</span>}
        {anon&&<span style={{fontSize:10,color:T.txD,background:T.bg4,padding:"1px 6px",borderRadius:4}}>{t("chat.pollAnon")}</span>}
      </div>}
      {options.map((opt,i)=>{
      ...
      <div style={{fontSize:10,color:T.txD,textAlign:"right"}}>
        {anon?t("chat.voteCount",{n:totalVotes})
          :totalVoters>0?<span onClick={()=>setShowVoters(true)} style={{cursor:"pointer",color:T.accent,fontWeight:500}}>{t("chat.voterCount",{n:totalVoters})} &rsaquo;</span>
          :t("chat.voterCount",{n:0})}
      </div>
      {/* Voter modal */}
      {showVoters&&!anon&&<div onClick={()=>setShowVoters(false)} style={{position:"fixed",inset:0,background:"rgba(0,0,0,.5)",zIndex:100,display:"flex",alignItems:"center",justifyContent:"center",padding:20}}>
        <div onClick={e=>e.stopPropagation()} style={{background:T.bg2,borderRadius:14,width:"100%",maxWidth:320,maxHeight:"70vh",overflow:"hidden",display:"flex",flexDirection:"column"}}>
          <div style={{display:"flex",alignItems:"center",justifyContent:"space-between",padding:"12px 16px",borderBottom:`1px solid ${T.bd}`}}>
            <span style={{fontSize:14,fontWeight:600,color:T.txH}}>{t("chat.voterList")}</span>
...
export const ChatView=({course,dept,roomId:roomIdProp,roleMap,mob})=>{
  ...
  const sendAnnounce=()=>{
    const txt=announceText.trim();if(!txt)return;
    sendMessage(`📢 **${t("chat.announceLabel")}**\n${txt}`,user);
  ...
  const _dateLabel=d=>{
    ...
    if(diff===0)return t("chat.today");if(diff===1)return t("chat.yesterday");
  ...
  const composeHeader=(icon,label,onSend,canSend)=>(
    <div style={{display:"flex",alignItems:"center",gap:8,padding:"8px 12px",borderBottom:`1px solid ${T.bd}`}}>
      ...
      <button onClick={()=>setCompose(null)} style={{background:"none",border:"none",color:T.txD,cursor:"pointer",fontSize:12,fontWeight:500}}>{t("common.cancel")}</button>
      <button onClick={onSend} disabled={!canSend}
        style={{padding:"5px 14px",borderRadius:6,border:"none",background:canSend?T.accent:T.bg4,color:canSend?"#fff":T.txD,fontSize:12,fontWeight:600,cursor:canSend?"pointer":"default",transition:"all .12s"}}>{t("chat.send")}</button>
    </div>
  );
  ...
  return(
    <div style={{flex:1,display:"flex",flexDirection:"column",overflow:"hidden"}}>
      <div ref={scrollRef} style={{flex:1,overflowY:"auto",overflowX:"hidden",WebkitOverflowScrolling:"touch",padding:"8px 0"}}>
        {loading&&<Loader msg={t("chat.loadingMessages")} size="sm"/>}
        {grp.map(m=>{
      ...
      {typingUsers.length>0&&<div style={{padding:"2px 14px",fontSize:11,color:T.txD,fontStyle:"italic"}}>
        {t("chat.typing",{users:typingUsers.join(t("chat.userSep"))})}
      </div>}

      {/* ── Compose: Code Block ── */}
      {compose==='code'&&<div style={{borderTop:`1px solid ${T.bd}`,background:T.bg2}}>
        {composeHeader(I.code,t("chat.codeBlock"),sendCode,!!codeBody.trim())}
        <div style={{padding:"8px 12px",display:"flex",flexDirection:"column",gap:6}}>
          <input value={codeLang} onChange={e=>setCodeLang(e.target.value)} placeholder={t("chat.codeLangPlaceholder")}
          ...
          <textarea value={codeBody} onChange={e=>setCodeBody(e.target.value)} placeholder={t("chat.codePlaceholder")} rows={5}
      ...
      {compose==='poll'&&<div style={{borderTop:`1px solid ${T.bd}`,background:T.bg2}}>
        {composeHeader(I.poll,t("chat.createPoll"),sendPoll,!!pollQ.trim()&&pollOpts.filter(o=>o.trim()).length>=2)}
        <div style={{padding:"8px 12px",display:"flex",flexDirection:"column",gap:6}}>
          <input value={pollQ} onChange={e=>setPollQ(e.target.value)} placeholder={t("chat.pollQuestionPlaceholder")}
          ...
          {pollOpts.map((o,i)=>(
            <div key={i} style={{display:"flex",gap:6,alignItems:"center"}}>
              ...
              <input value={o} onChange={e=>{const a=[...pollOpts];a[i]=e.target.value;setPollOpts(a);}} placeholder={t("chat.pollOption",{n:i+1})}
          ...
          {pollOpts.length<6&&<button onClick={()=>setPollOpts(p=>[...p,""])}
            ...
            <span style={{display:"flex"}}>{I.plus}</span>{t("chat.addOption")}
          </button>}
          <div style={{display:"flex",gap:12,paddingTop:4,borderTop:`1px solid ${T.bd}`,marginTop:2}}>
            {[{key:"multi",label:t("chat.pollMulti"),val:pollMulti,set:setPollMulti},{key:"anon",label:t("chat.pollAnonVote"),val:pollAnon,set:setPollAnon}].map(s=>(
      ...
      {compose==='announce'&&<div style={{borderTop:`1px solid ${T.bd}`,background:T.bg2}}>
        {composeHeader(I.mega,t("chat.announce"),sendAnnounce,!!announceText.trim())}
        <div style={{padding:"8px 12px"}}>
          <div style={{padding:"10px 12px",borderRadius:8,border:`2px solid ${T.orange||"#d4843e"}30`,background:`${T.orange||"#d4843e"}08`}}>
            <div style={{display:"flex",alignItems:"center",gap:6,marginBottom:6}}>
              ...
              <span style={{fontSize:12,fontWeight:700,color:T.orange||"#d4843e"}}>{t("chat.announce")}</span>
            </div>
            <textarea value={announceText} onChange={e=>setAnnounceText(e.target.value)} placeholder={t("chat.announcePlaceholder")} rows={3}
        ...
        <div style={{display:"flex",alignItems:"center",gap:6,padding:"3px 3px 3px 12px",borderRadius:mob?20:8,background:T.bg3,border:`1px solid ${T.bd}`}}>
          <div style={{position:"relative"}}>
            ...
            {menuOpen==='actions'&&<div style={{position:"absolute",bottom:36,left:0,background:T.bg2,border:`1px solid ${T.bd}`,borderRadius:12,padding:6,zIndex:10,boxShadow:"0 4px 16px rgba(0,0,0,.25)",minWidth:160}}>
              {[
                {icon:I.mask,label:t("chat.emoji"),action:()=>setMenuOpen('emoji')},
                {icon:I.code,label:t("chat.codeBlock"),action:()=>{closeMenu();setCompose('code');}},
                {icon:I.poll,label:t("chat.createPoll"),action:()=>{closeMenu();setCompose('poll');}},
                {icon:I.mega,label:t("chat.announce"),action:()=>{closeMenu();setCompose('announce');}},
              ].map(item=>(
            ...
            {menuOpen==='emoji'&&<div style={{position:"absolute",bottom:36,left:0,background:T.bg2,border:`1px solid ${T.bd}`,borderRadius:10,padding:8,zIndex:10,boxShadow:"0 4px 16px rgba(0,0,0,.25)"}}>
              <div style={{display:"flex",alignItems:"center",gap:6,marginBottom:6,paddingBottom:6,borderBottom:`1px solid ${T.bd}`}}>
                ...
                <span style={{fontSize:12,fontWeight:600,color:T.txD}}>{t("chat.emoji")}</span>
              </div>
          ...
          <input value={inp} onChange={e=>{setInp(e.target.value);setTyping(!!e.target.value.trim());}} onKeyDown={e=>e.key==="Enter"&&(e.preventDefault(),send())} placeholder={t("chat.messagePlaceholder")} style={{flex:1,padding:"8px 0",border:"none",background:"transparent",color:T.txH,fontSize:14,outline:"none",fontFamily:"inherit"}}/>
