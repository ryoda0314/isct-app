...
export const SetupView = ({ onComplete, onSkip, onDemo, mob }) => {
  ...
  const [step, setStep] = useState(0);      // signup wizard: 0=ISCT, 1=Portal, 2=Profile
  ...
  const goBack = () => {
    ...
    if (mode === "signup") { setStep(s => s - 1); return; }
  };
  const nextStep = () => { setError(null); setStep(s => s + 1); };

  /* ── Submit (shared by login & signup) ── */
  ...
  const Header = ({ showBack, progress }) => (
    <div style={{ paddingTop: "env(safe-area-inset-top)", background: T.bg2, borderBottom: `1px solid ${T.bd}`, flexShrink: 0 }}>
      <div style={{ height: 46, display: "flex", alignItems: "center", justifyContent: "center", position: "relative", padding: "0 12px" }}>
        {showBack && (
          <button onClick={goBack} style={{ position: "absolute", left: 12, background: "none", border: "none", color: T.txD, cursor: "pointer", display: "flex", padding: 4 }}>
            {I.back}
          </button>
        )}
  ...
  const PrimaryBtn = ({ onClick, children, disabled, muted }) => (
    <button onClick={onClick} disabled={disabled} style={{
      width: "100%", padding: "14px 0", borderRadius: 12, border: "none",
      background: muted ? T.bg3 : (disabled ? `${T.accent}40` : T.accent),
      color: muted ? T.txD : "#fff",
  ...
  const renderWelcome = () => (
    <div style={{
      ...
      justifyContent: "center", padding: mob ? "20px 24px" : "40px",
    }}>
      <div style={{ maxWidth: 400, width: "100%", textAlign: "center", animation: "fadeIn .3s ease" }}>
        {/* Logo */}
        <div style={{
        ...
        <p style={{ fontSize: 14, color: T.txD, margin: "0 0 28px", lineHeight: 1.6 }}>
          LMS・教務Webと連携して<br />大学生活をもっと便利に
        </p>
        ...
        <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 10, marginBottom: 32 }}>
          {FEATURES.map((f, i) => (
            <div key={i} style={{
              ...
              border: `1px solid ${T.bd}`, background: T.bg2, textAlign: "center",
            }}>
  ...
  const renderLogin = () => (
    <Body>
      ...
      <p style={{ fontSize: 13, color: T.txD, margin: "0 0 20px", lineHeight: 1.5 }}>
        ISCT LMSのアカウントでログインします
      </p>

      <ErrorBanner />

      <div style={{ display: "flex", flexDirection: "column", gap: 14, padding: 14, borderRadius: 12, border: `1px solid ${T.bd}`, background: T.bg2 }}>
        ...
        <InputField
          ...
          note="2段階認証アプリ設定時に表示されたキー"
        />
      </div>

      <div style={{ marginTop: 24 }}>
        <PrimaryBtn onClick={handleSubmit} disabled={!hasIsct}>ログイン</PrimaryBtn>
      </div>
      ...
      <div style={{ textAlign: "center", marginTop: 20 }}>
        ...
        <button onClick={() => { setMode("signup"); setStep(0); setError(null); }} style={{
          ...
          fontWeight: 600, cursor: "pointer", padding: "0 4px",
        }}>新規登録</button>
      </div>
    </Body>
  );
  ...
  const renderSignupIsct = () => (
    <Body>
      ...
      <div style={{ marginTop: 24 }}>
        <PrimaryBtn onClick={nextStep}>{hasIsct ? "次へ" : "スキップして次へ"}</PrimaryBtn>
      </div>
  ...
  const renderSignupPortal = () => (
    <Body>
      ...
      <div style={{ marginTop: 24 }}>
        <PrimaryBtn onClick={nextStep}>{hasPortal ? "次へ" : "スキップして次へ"}</PrimaryBtn>
      </div>
  ...
  const renderSignupProfile = () => (
    <Body>
      ...
      <div style={{ marginTop: 24 }}>
        <PrimaryBtn onClick={handleSubmit} muted={!hasAny}>
  ...
  return (
    <div style={PAGE}>
      <Header showBack={showBack} progress={progress} />
      {mode === null && renderWelcome()}
      {mode === "login" && renderLogin()}
      {mode === "signup" && step === 0 && renderSignupIsct()}
      {mode === "signup" && step === 1 && renderSignupPortal()}
      {mode === "signup" && step === 2 && renderSignupProfile()}
      <Footer />
      <style>{GLOBAL_CSS}</style>
    </div>
  );
